...
class UpdateUserRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('status')) {
            ...
            // Handle string '1'/'0', integer 1/0, boolean true/false
            if (is_string($status)) {
                $this->merge([
                    'status' => in_array(strtolower($status), ['1', 'true', 'on', 'yes'], true),
                ]);
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            if ($this->has('cpf')) {
                ...
                // Check if CPF already exists (without formatting) and ignore current user
                $cpfCleaned = preg_replace('/\D/', '', $this->cpf);
                $exists = User::where('cpf', $cpfCleaned)
